...
import { invoicesTb, userT, systemStatsTb } from '../schema'
import { LibSQLDatabase } from 'drizzle-orm/libsql'
import * as schema from '../schema'

export async function calculateAndStoreSystemStats(db: LibSQLDatabase<typeof schema>) {
  console.log('Starting system stats calculation...')

  const [userCountResult] = await db.select({ count: count() }).from(userT)
  const [invoiceCountResult] = await db.select({ count: count() }).from(invoicesTb)
